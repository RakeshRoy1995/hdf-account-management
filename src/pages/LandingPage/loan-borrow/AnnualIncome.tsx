...
export default function AnnualIncome({setsingleData, singleData }:any) {

  const [options, setOptions] = useState(singleData?.personAnnualIncomes || [""]); // Default options for select-type questions
  ...
  const handleOptionChange = (index: any, value: any, name: any) => {
    ...
    setsingleData({
      ...singleData,
      ["personAnnualIncomes"]: updatedOptions,
    });
  };
  ...
  const removeOption = async(index: any) => {
    ...
    if (found?.personAnnualIncomeId) {
      const page_list = `${API_URL}/person-annual-incomes/${found?.personAnnualIncomeId}`;
      const option = {
        method: "DELETE",
      };
    ...
    setsingleData({
      ...singleData,
      ["personAnnualIncomes"]: options.filter((_, i) => i !== index),
    });
  };
  return (
        ...
        <>
          <div className="flex flex-col relative">
            ...
            <input
              ...
              value={singleData?.personAnnualIncomes?.length && singleData?.personAnnualIncomes[index]?.name}
              onChange={(e) => {
          ...
          <div className="flex flex-col relative">
            ...
            <input
              ...
              name="amount"
              placeholder="Amount"
              value={singleData?.personAnnualIncomes?.length && singleData?.personAnnualIncomes[index]?.amount}
              onChange={(e) => {
                handleOptionChange(index, e.target.value, e.target.name);
              }}
              className="border p-4 rounded appearance-none h-14"
