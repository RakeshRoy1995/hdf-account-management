...
export default function Liability({ setsingleData, singleData }: any) {
  const [options, setOptions] = useState(singleData?.personLiabilities || [""]); // Default options for select-type questions
  ...
  const handleOptionChange = (index: any, value: any, name: any) => {
    ...
    setsingleData({
      ...singleData,
      ["personLiabilities"]: updatedOptions,
    });
  };
  ...
  const removeOption = async (index: any) => {
    ...
    if (found?.personLiabilityId) {
      const page_list = `${API_URL}/person-liabilities/${found?.personLiabilityId}`;
      const option = {
        method: "DELETE",
      };
    ...
    setsingleData({
      ...singleData,
      ["personLiabilities"]: options.filter((_, i) => i !== index),
    });
  };
          ...
          <>
            <div className="flex flex-col relative">
              ...
              <input
                ...
                placeholder="Name"
                value={
                  singleData?.personLiabilities?.length &&
                  singleData?.personLiabilities[index]?.name
                }
                onChange={(e) => {
            ...
            <div className="flex flex-col relative">
              ...
              <input
                ...
                value={
                  singleData?.personLiabilities?.length &&
                  singleData?.personLiabilities[index]?.amount
                }
                onChange={(e) => {
                  handleOptionChange(index, e.target.value, e.target.name);
                }}
