...
export default function PersonLand({ setsingleData, singleData }: any) {
  const [options, setOptions] = useState(singleData?.personLands || [""]); // Default options for select-type questions
  ...
  const removeOption = async(index: any) => {
    ...
    if (found?.personLandId) {
      const page_list = `${API_URL}/person-lands/${found?.personLandId}`;
      const option = {
        method: "DELETE",
      };
    ...
    setsingleData({
      ...singleData,
      ["personLands"]: options.filter((_, i) => i !== index),
    });
  };
          ...
          <>
            <div className="flex flex-col relative">
              ...
              <input
                ...
                name="name"
                placeholder="Name"
                value={singleData?.personLands?.length && singleData?.personLands[index]?.name}
                onChange={(e) => {
            ...
            <div className="flex flex-col relative">
              ...
              <input
                ...
                value={ singleData?.personLands?.length && singleData?.personLands[index]?.quantity}
                onChange={(e) => {
                  handleOptionChange(index, e.target.value, e.target.name);
                }}
                className="border p-4 rounded appearance-none h-14"
            ...
            <div className="flex flex-col relative">
              ...
              <input
                ...
                name="amount"
                placeholder="Amount"
                value={ singleData?.personLands?.length && singleData?.personLands[index]?.amount}
                onChange={(e) => {
                  handleOptionChange(index, e.target.value, e.target.name);
                }}
                className="border p-4 rounded appearance-none h-14"
