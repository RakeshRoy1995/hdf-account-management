...
const UpdateButtonGL = ({ setparentName, searchData, loading }: any) => {
  const { t } = useTranslation();

  const data: any = accessPermission();

  const tmpAddBtn = data?.permission?.find(
    (d: any) => d.name == "Update" && d.method == "PUT",
  );

  const updateBtn = tmpAddBtn?.checked == false ? false : true;
          ...
          <div>
            <div>
              <div className="flex justify-end mt-5">
                <button
                  type="button"
                  onClick={(e: any) => setparentName(searchData?.glAccountNo)}
